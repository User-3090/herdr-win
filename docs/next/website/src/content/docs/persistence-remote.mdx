...
Native Windows clients can attach to those Linux and macOS hosts. This fork also preserves its pre-provisioned Windows-host adapter for the Herdr sandbox layout at `C:\HerdrSandbox\runtime\herdr\herdr.exe`; automatic install and live handoff remain Unix-host-only.
