...
| `herdr --remote` to Linux/macOS and Windows hosts with `herdr.exe` on the SSH user's `PATH` | beta |
...
| Remote clipboard image paste to agents | beta |
...
For `herdr --remote`, Herdr reads registered PNG and DIB clipboard formats on Windows and sends a bounded PNG to the remote server. The default shortcut is `ctrl+v`; set `keys.remote_image_paste` to change or disable it. A pasted or dropped absolute image path is transported in the same way. This bridge is remote-only and falls back to normal input when no supported clipboard image is available.
...
Preview packages include a pinned Microsoft app-local ConPTY runtime because the system ConPTY on older Windows 10 builds drops Kitty keyboard protocol sequences used by agents such as Kimi and Pi. Set `HERDR_WINDOWS_CONPTY=system` before starting Herdr only when diagnosing a compatibility problem with the bundled runtime.
...
During a remote attach, use `ctrl+v` for clipboard images. Herdr accepts registered PNG, CF_DIBV5, and CF_DIB data, converts DIB data to a bounded PNG, and pastes a staged remote path into the focused pane.
...
The native Windows binary can act as a thin client for Linux and macOS Herdr hosts and for Windows hosts where the SSH user's `PATH` resolves `herdr.exe`:

```powershell
herdr --remote workbox
```

Herdr does not use its control-socket connection reuse path on Windows. It still adds SSH keepalives, but password authentication may prompt for more than one setup command. Key or Windows `ssh-agent` authentication is recommended. Linux and macOS hosts support discovery and optional installation; Windows hosts start exactly the PATH-resolved `herdr.exe`, never install a remote binary automatically, and do not support live handoff.
