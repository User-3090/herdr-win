...
| `herdr --remote` to Linux/macOS and pre-provisioned Windows hosts | beta |
...
The native Windows binary can act as a thin client for Linux and macOS Herdr hosts and for this fork's pre-provisioned Windows sandbox host:

```powershell
herdr --remote workbox
```

Herdr does not use its control-socket connection reuse path on Windows. It still adds fallback SSH keepalives, but password authentication may prompt for more than one setup command. Key or Windows `ssh-agent` authentication is recommended. Linux and macOS hosts support discovery and optional installation; Windows hosts must already contain the sandbox Herdr executable and do not support live handoff.
