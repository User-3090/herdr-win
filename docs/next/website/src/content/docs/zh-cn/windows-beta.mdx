...
| 窗格屏幕历史 | 测试版 |
| 嵌套启动覆盖 | 测试版 |
| 连接 Linux/macOS 和预配置 Windows 主机的 `herdr --remote` | 测试版 |
...
原生 Windows 二进制可以作为 Linux 和 macOS Herdr 主机以及本分支预配置 Windows 沙盒主机的瘦客户端:

```powershell
herdr --remote workbox
```

Herdr 在 Windows 上不使用控制套接字连接复用。它仍会添加备用 SSH keepalive,但密码认证可能会为多个设置命令重复提示。建议使用密钥或 Windows `ssh-agent` 认证。Linux 和 macOS 主机支持检测和可选安装;Windows 主机必须已经包含沙盒 Herdr 可执行文件,并且不支持实时交接。
