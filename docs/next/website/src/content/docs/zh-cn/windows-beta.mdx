...
| 窗格屏幕历史 | 测试版 |
| 嵌套启动覆盖 | 测试版 |
| 连接 Linux/macOS 和 SSH 用户 `PATH` 中有 `herdr.exe` 的 Windows 主机的 `herdr --remote` | 测试版 |
...
使用 `herdr --remote` 时,Herdr 会在 Windows 上读取已注册的 PNG 和 DIB 剪贴板格式,并向远程服务器发送受大小限制的 PNG。默认快捷键是 `ctrl+v`;可通过 `keys.remote_image_paste` 更改或禁用。粘贴或拖放的绝对图像路径也会以同样方式传输。此桥接仅用于远程连接;没有受支持的剪贴板图像时会回退到普通输入。
...
旧版 Windows 10 的系统 ConPTY 会丢弃 Kimi 和 Pi 等智能体使用的 Kitty 键盘协议序列,因此预览包包含固定版本的 Microsoft 应用本地 ConPTY 运行时。仅在诊断捆绑运行时的兼容性问题时,才应在启动 Herdr 前设置 `HERDR_WINDOWS_CONPTY=system`。
...
远程连接期间,使用 `ctrl+v` 粘贴剪贴板图像。Herdr 接受已注册的 PNG、CF_DIBV5 和 CF_DIB 数据,把 DIB 数据转换为受大小限制的 PNG,并将暂存的远程路径粘贴到当前聚焦的窗格中。
...
原生 Windows 二进制可以作为 Linux 和 macOS Herdr 主机以及 SSH 用户的 `PATH` 能解析 `herdr.exe` 的 Windows 主机的瘦客户端:

```powershell
herdr --remote workbox
```

Herdr 在 Windows 上不使用控制套接字连接复用。它仍会添加 SSH keepalive,但密码认证可能会为多个设置命令重复提示。建议使用密钥或 Windows `ssh-agent` 认证。Linux 和 macOS 主机支持检测和可选安装;Windows 主机只会启动从 SSH 用户 `PATH` 解析出的 `herdr.exe`,不会自动安装远程二进制文件,也不支持实时交接。
