...
The managed installation has one launcher at `bin\herdr.exe`; it starts the selected immutable payload directly. Runtime directories do not contain another launcher. Setup replaces an idle launcher immediately or stages a validated replacement until the final managed payload exits, then the existing helper publishes it atomically and prunes every exact unleased runtime except Active and optional Pending. Busy or ambiguous content is preserved. A hard process-tree kill leaves pending work recoverable for the next safe launch or setup.

Setup updates only an exact current managed installation. If it reports an incompatible existing installation, uninstall **Herdr Win** from **Windows Settings → Apps → Installed apps**, then run setup again. Setup preserves that directory and does not migrate, back up, or remove it.
