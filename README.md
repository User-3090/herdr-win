...
The English-only, keyboard-operable Windows setup consistently presents the distribution as **Herdr Win** while the repository and release slug remains `herdr-win`. Welcome and Finish show the current reviewed Herdr base version and identify Herdr Win as an unofficial distribution of that stable release plus the maintained Windows patches. Welcome does not expose or offer a choice for the fixed `%LOCALAPPDATA%\Programs\Herdr` destination. Setup requires no administrator privileges, presents the Apache-2.0 license before modifying files, adds its stable `bin` directory to your user `PATH`, and registers Herdr in **Windows Settings → Apps → Installed apps**. Finish shows the first `herdr` command and separate optional links to the [herdr-win fork](https://github.com/User-3090/herdr-win) and [official Herdr project](https://github.com/ogulcancelik/herdr); setup never opens either automatically. During the progress page, the managed PowerShell helper validates and transactionally activates the immutable runtime, then updates PATH, Installed Apps, and managed skill copies; this can take several seconds and is not an artificial wait. The same license remains available as `LICENSE.txt`. Setup copies upstream's canonical agent skill to `%USERPROFILE%\.agents\skills\herdr\SKILL.md` and, when Claude Code is detected, to its configured or default `.claude\skills\herdr\SKILL.md`. Install and update overwrite only `SKILL.md`; every sibling file and directory remains untouched. The executable and command remain `herdr.exe` and `herdr`.
