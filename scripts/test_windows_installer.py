...
class WindowsInstallerStaticTests(unittest.TestCase):
    ...
    def test_installer_owns_the_cross_agent_skill_lifecycle(self) -> None:
        ...
        self.assertIn('$skillHashManifestValidationText = $skillHashManifestText.Replace("`r`n", "`n")', packager)
        self.assertIn('[IO.File]::WriteAllBytes($canonicalSkillHashManifest, $canonicalSkillHashManifestBytes)', packager)
        self.assertIn('"/DARG_SKILL_HASH_MANIFEST=$canonicalSkillHashManifest"', packager)
