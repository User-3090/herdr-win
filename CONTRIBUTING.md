...
manual release workflow builds a fresh checkout of recorded `BASE`, so product
changes must be represented by the canonical queue.
...
`website/preview.json` is generated by the manual workflow after release publication.
Do not hand-edit it or include its manifest-only commit in release source
...
and verified independently. Runtime builds retain Herdr's preview-channel identity
for update compatibility, while fork release presentation calls the artifacts
